...
<p align="center">
  ...
  AST感知切块 · BM25极速全文搜索 · MCP原生协议 · 30MB零依赖<br>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/大小-30MB-10B981" alt="大小">
...
| 跨平台部署 | 需要安装运行时（Python/Node） | 30MB 单二进制，零依赖，下载即用 |
...
> 在 3.1.x 已修复的 `--all` 中文全文搜索、`file` 的 `--max-tokens`、中文/空格目录路径基础上，3.2.0 进一步强化 AI 场景下的搜索体验：

### AI 智能路由（`ai` 命令）
- **符号快路径**：`sts-x ai "符号名"` 绕过 BM25 直接精确匹配符号，比 `search` 更准更快，自动 `locate`。
- **自然语言兜底**：`sts-x ai "一段描述"` 自动路由到 `expand` 做 BM25 匹配（纯描述句可能返回 0，此时回退 `search`/`file`）。
- 一个 `ai` 覆盖符号定位与自然语言查询，AI 无需再猜该用哪个子命令。

### 聚合去重
同名多命中自动聚合，响应体积下降约 **62%**，在大型多 crate 项目中尤其明显。

### Workspace 根探测
跨多个 crate 自动爬到真正的项目根，护栏：爬升深度 ≤8、遇 `.stsx-root` 即停、禁止爬 `$HOME`，索引更准且不越界。

### 依赖精简
移除 `axum`，净依赖减少，二进制更轻、构建更简单。
...
| **AI** | `ai "query"` | —（CLI 专用） | 符号 / 自然语言 | 符号→locate 精确；描述→expand（NL 可能返回 0，回退 search/file） |
...
|------|------|
| **版本** | v3.2.0 |
| **二进制大小** | macOS ~30MB / Windows ~30MB（strip 后） |
| **搜索延迟** | 0–2ms（千级文件） |
| **索引引擎** | Tantivy BM25（自定义 code 分词器） |
| **AST 解析** | tree-sitter（9 种语言） |
| **MCP 服务** | 内置轻量 HTTP 服务（可选启用），RESTful 设计（`/search` + `/file` 双工具） |
| **AI 命令 `ai`** | v3.2.0 新增：符号快路径（精确 locate）+ 自然语言路由（→expand） |
| **聚合去重** | v3.2.0 新增：同名多命中聚合，响应体积降 ~62% |
| **Workspace 根探测** | v3.2.0 新增：跨多 crate 自动爬真根（护栏：深度≤8 / `.stsx-root` 即停 / 禁爬 $HOME） |
| **外部依赖** | 零 |
| **默认输出** | JSON（AI 原生格式） |
...
```bash
# 默认构建（BM25 模式，约 30MB macOS / 30MB Windows）
cargo build --release
...
│   ├── server/            # MCP HTTP 服务（可选启用）
│   ├── router.rs          # AI 智能路由（v3.2.0：ai 命令）
│   ├── filesearch/        # 零索引文件搜索（file 子命令）
